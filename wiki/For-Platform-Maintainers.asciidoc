...
| `HEURISTIC` / `DATAFLOW` / `PROTOCOL` | `level` from severity
  (ERROR→error, WARN→warning, INFO→note); `rank` 50 / 80 / 70
| evidence class, tier, note, counter-condition | `result.properties`
  (`evidenceClass`, `tier`, `evidenceNote`, `counterCondition`) — round-trips
| message | `message.text`; `SPECULATIVE` gets " (heuristic — not a verdict)"
  appended
| rule metadata | `tool.driver.rules[]` reporting descriptors with
  `defaultConfiguration.level` and `properties.tier`
|===
