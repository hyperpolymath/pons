...
| `pons-core` | The engine: file discovery, language registry, parsing, the CFG
  + dataflow, the `Finding` model, suppression, and the three reporters. *No
  rules live here.*
| `pons-rules` | *The catalogue.* One module per tier (`t0/`…`t3/`), a `registry`
  that inventories every rule, and `rules/` data files (queries + metadata).
